...
**<font color=orange size=4>如果你只是想玩这个数据包，不想二次开发，请直接下载在 release 发布的正式版本中的 `GlobalShop_v?.?.?.zip`，它就是对应版本源代码生成的数据包。</font>**

### 2. 如何选择本数据包的版本？
请查看 [version_records.md](version_records.md)，里面记录了本数据包的所有版本和对应的生效 minecraft 版本；

本数据包的版本号初始为 `v3.0.1`，其中 `v3.0` 是大版本号。只有当 minecraft 推出新版本后，作者才会推出新的大版本号来适配新的 minecraft 版；其中 `1` 是小版本号，仅供修复错误使用。（例如，本数据包的 `v3.0.1` 和 `v3.0.8` 的生效 minecraft 版本相同，因为它们的大版本号都是 `v3.0`，但后者修复了一些错误，你应该选择后者而不是前者。）
...
4. 切换到 `translation` 工作表，棕色行说明可以引用自一个通用翻译键，不需要翻译；如果是黑色行则需要翻译，请在 `H` 列（`new_language`）书写翻译文本；如果在 `en_us` 列和 `zh_cn` 列没有文本则说明这是个空行，只起到分隔作用，无需翻译。在 `D` 列和 `E` 列还有描述或截图帮助你理解文本在游戏中的显示位置；
5. 切换到 `common` 工作表来翻译通用翻译键，相同的文本被合并为翻译键来减少翻译工作量。请在 `E` 列（`new_language`）进行翻译；
...
## 为什么数据包提示“全球商店出现错误，部分指令运行异常。”，然后就不运行了？
在 minecraft 的命令系统中，存在“最大连锁执行命令数”的概念，游戏不允许一次执行太多命令。在原版中，这个值默认是 65536。本数据包参考了这个值来约束自身的命令执行数量。只要不修改这个值，本数据包就不会出现命令执行不完的情况。
...
<font color=red>如果你不顾数据损坏，执意要启动</font>，可以使用 `/function global_shop:settings/reinit_anyway` 强制重新初始化。商店会尝试启动，但由于数据可能是损坏的，商店运行时随时可能出现意料之外的问题，请谨慎使用。