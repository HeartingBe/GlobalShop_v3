#> global_shop:storage/store_manager/get_player_shop_return_list_page
# @brief 返回玩家退回的物品列表中，从下标 beginIndex 开始的数据, beginIndex < 0 或者溢出最大范围的部分填充空项
# @param beginIndex 开始下标，从 Menu 的记分板取得
# @param g_itemsToDisplay 目标位置，将结果写入该路径
# @executor Menu

# 先取出玩家退回的物品列表，存储在 global_shop:storage g_tempList
...
function global_shop:storage/store_manager/cache_player_shop_return_list with storage global_shop:common temp
