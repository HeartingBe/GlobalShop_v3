# @brief 获取 g_tempList[index] 并将其尾插到 g_itemsToDisplay
...
execute store result score temp3 glbs_common run data get storage global_shop:storage g_tempList
...
# g_itemsToDisplay.push_back(g_tempList[index]);
execute store result storage global_shop:common temp.index int 1 run scoreboard players get temp2 glbs_common
function global_shop:storage/store_manager/get_cache_list_page/get_elem_and_append/1 with storage global_shop:common temp