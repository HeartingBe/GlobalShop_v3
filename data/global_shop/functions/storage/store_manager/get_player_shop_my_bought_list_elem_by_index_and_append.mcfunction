# @brief 获取 g_playerBoughtListMap[uid][index] 并将其尾插到 g_itemsToDisplay
# @macro {uid: int}
...
$execute store result score temp3 glbs_common run data get storage global_shop:storage g_playerBoughtListMap.$(uid)
...
# 在父函数就已经构造好宏参数的 uid, 补充 index 即可
execute store result storage global_shop:common temp.index int 1 run scoreboard players get temp2 glbs_common
function global_shop:storage/store_manager/get_player_shop_my_bought_list_elem_by_index_and_append/1 with storage global_shop:common temp