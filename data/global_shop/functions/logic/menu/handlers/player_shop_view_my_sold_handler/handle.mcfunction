...
# 判断玩家右键
   ...
   execute store result score temp glbs_common run function global_shop:logic/menu/handlers/menu_handler/get_back_action
      # 返回玩家商店主菜单
      execute if score temp glbs_common = Action::RIGHT_CLICK glbs_common run return run function global_shop:logic/menu/handlers/player_shop_view_my_sold_handler/handle/player_back
...
# 玩家旋转
   ...
   # 转动更新
   execute unless score g_columnDiff glbs_common matches 0 run function global_shop:logic/menu/handlers/player_shop_view_my_sold_handler/rotate_and_update_items
...
# 判断玩家看向新的物品实体
   ...
   execute unless score targetOrder glbs_common = @s glbs_last_view_order run return run function global_shop:logic/menu/handlers/player_shop_view_my_sold_handler/target_new_item
