# @brief 浏览历史出售记录时左键，直接向玩家发送出售记录的信息
# @executor Menu
...
# 用 glbs_last_view_id 获取物品信息
   ...
   function global_shop:storage/store_manager/get_player_sold_list_elem_by_uid_and_id

# 物品不存在，刷新显示（不可能不存在）
#-

# 向玩家 tellraw 发送该物品信息
execute on passengers if entity @s[type=minecraft:player] run function global_shop:logic/menu/handlers/player_shop_view_my_sold_handler/handle/send_item_info_to_player
