...
# 对比 id 判断是否为玩家自己出售的物品
   ...
   # 是其他玩家出售的物品，判断金钱是否足够
      ...
      # 钱不够
      execute if score temp1 glbs_common < temp2 glbs_common run return run function global_shop:logic/menu/handlers/player_shop_view_all_handler/handle/player_left_click_confirm/money_not_enough
...
# 钱足够
   ...
   # 将买家的钱加到卖家的收益上
      ...
      function global_shop:logic/menu/handlers/player_shop_view_all_handler/handle/player_left_click_confirm/macro_add_seller_income with storage global_shop:common temp
      # 更新卖家的出售物品数量
