#> global_shop:logic/menu/handlers/player_use_money_item_exchange_handler/handle/player_left_click/exchange_1
# @executor player

# 是否被禁用
execute if data storage global_shop:storage g_cashInfo[1].disable run return run function global_shop:logic/menu/handlers/player_use_money_item_exchange_handler/handle/player_left_click/exchange_n/cash_disabled
...
data modify storage global_shop:common g_itemData set from storage global_shop:storage g_cashInfo[1].item
    ...
    data modify entity 00000d3a-0000-0d3a-0000-17cc0000edfa text set value '[{"type":"nbt","storage":"global_shop:storage","nbt":"g_lang.item_frame.cash_value","color":"yellow"},{"text":": ","color":"yellow},{"nbt":"g_cashInfo[1].item.tag.global_shop.price","storage":"global_shop:storage","color":"aqua"}]'
