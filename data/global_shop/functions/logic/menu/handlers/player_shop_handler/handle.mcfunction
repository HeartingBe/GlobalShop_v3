...
# 判断玩家看向新的物品实体
   ...
   # 看向新的物品时，检查物品。存在时展示信息，不存在时刷新
      # glbs_last_action_target_order 记录 0 说明 看向的物品为空项 或者 没有看向物品 或者 物品不存在
   execute unless score temp glbs_common = @s glbs_last_action_target_order run return run function global_shop:logic/menu/handlers/player_shop_handler/target_new_item
