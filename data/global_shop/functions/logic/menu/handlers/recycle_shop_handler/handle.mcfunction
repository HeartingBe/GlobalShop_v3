...
# 判断玩家右键
   ...
   execute store result score temp glbs_common run function global_shop:logic/menu/handlers/menu_handler/get_back_action
      # 返回主菜单
      execute if score temp glbs_common = Action::RIGHT_CLICK glbs_common run return run function global_shop:logic/menu/handlers/recycle_shop_handler/handle/player_back
...
# 玩家旋转
   ...
   # 转动更新
   execute unless score g_columnDiff glbs_common matches 0 run function global_shop:logic/menu/handlers/recycle_shop_handler/rotate_and_update_items
...
# 判断玩家看向新的物品实体
   ...
   # 看向新的物品时，检查物品。存在时展示信息，不存在时刷新
      # glbs_last_action_target_order 记录 0 说明 看向的物品为空项 或者 物品不存在
   execute unless score temp glbs_common = @s glbs_last_action_target_order run return run function global_shop:logic/menu/handlers/recycle_shop_handler/target_new_item
...
# 判断玩家左键
   ...
   execute unless score temp glbs_common = Action::NO_ACTION_THIS_PLAYER glbs_common run function global_shop:logic/menu/handlers/recycle_shop_handler/handle/player_left_click