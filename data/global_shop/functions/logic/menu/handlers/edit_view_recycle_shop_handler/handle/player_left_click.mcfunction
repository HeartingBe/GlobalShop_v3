...
# 用 glbs_last_view_id 获取物品信息
   ...
   function global_shop:storage/store_manager/get_recycle_shop_list_elem_by_id

# 物品不存在，刷新显示
execute unless data storage global_shop:common g_itemData run return run function global_shop:logic/menu/handlers/edit_view_recycle_shop_handler/handle/item_not_exist

# 玩家左键确认
   # index glbs_common 在下面这个函数中还要使用
execute if score @s glbs_last_action = Action::LEFT_CLICK_CONFIRM glbs_common run return run function global_shop:logic/menu/handlers/edit_view_recycle_shop_handler/handle/player_left_click_confirm
...
# 玩家首次左键
   ...
   scoreboard players operation order glbs_common = @s glbs_last_view_order
    ...
    # 删除物品的确认提示
    execute on passengers if entity @s[type=minecraft:player] run function global_shop:logic/menu/handlers/edit_view_recycle_shop_handler/handle/player_left_click/tip