# @brief 玩家浏览退还的物品并看向新的物品时高亮显示该物品。获取该物品 id 并写入 Menu
...
# 是被看的物品（只可能有一个物品展示实体执行到），检查是否存在
   ...
   execute on vehicle on passengers if entity @s[type=minecraft:player] store result score uid glbs_common run scoreboard players get @s glbs_uid
   execute on vehicle run function global_shop:storage/store_manager/get_return_list_elem_by_uid_and_id
      # 物品不存在
