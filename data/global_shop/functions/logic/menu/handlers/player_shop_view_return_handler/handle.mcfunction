...
# 判断玩家右键
   ...
   execute store result score temp glbs_common run function global_shop:logic/menu/handlers/menu_handler/get_back_action
      # 返回玩家商店主菜单
      execute if score temp glbs_common = Action::RIGHT_CLICK glbs_common run return run function global_shop:logic/menu/handlers/player_shop_view_return_handler/handle/player_back
...
# 玩家旋转
   ...
   # 转动更新
   execute unless score g_columnDiff glbs_common matches 0 run function global_shop:logic/menu/handlers/player_shop_view_return_handler/rotate_and_update_items
...
# 判断玩家看向新的物品实体
   ...
   execute unless score targetOrder glbs_common = @s glbs_last_view_order run return run function global_shop:logic/menu/handlers/player_shop_view_return_handler/target_new_item
   
# 判断玩家左键 玩家浏览自己正在出售的物品暂无左键逻辑
   # 获取左键情况
   ...
   execute unless score temp glbs_common = Action::NO_ACTION_THIS_PLAYER glbs_common run function global_shop:logic/menu/handlers/player_shop_view_return_handler/handle/player_left_click