...
# 取玩家设置页面数据
function global_shop:storage/store_manager/get_player_setting_list_page
# 取角度差 temp
...
# 更改模式
scoreboard players operation @s glbs_mode = Mode::PLAYER_SETTING glbs_common
# 通知
execute on passengers if entity @s[type=minecraft:player] run title @s actionbar [{"type":"nbt","storage":"global_shop:storage","nbt":"g_lang.change_mode.player_settings","color":"aqua"}]
# 生成物品展示实体
