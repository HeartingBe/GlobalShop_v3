...
execute if score temp glbs_common matches 1.. on passengers if entity @s[type=minecraft:player] run return run function global_shop:logic/menu/handlers/menu_handler/change_mode/edit_cash_exchange/other_op
# 清除上一个页面，初始化自身数据，准备加载此页面
function global_shop:logic/menu/handlers/menu_handler/change_mode/reset_state
# 读取货币的物品数据
function global_shop:logic/menu/handlers/menu_handler/change_mode/edit_cash_exchange/get_cashinfo_list
# 取角度差 temp
...
# 通知
execute on passengers if entity @s[type=minecraft:player] run function global_shop:logic/menu/handlers/menu_handler/change_mode/edit_cash_exchange/success
# 生成物品展示实体
