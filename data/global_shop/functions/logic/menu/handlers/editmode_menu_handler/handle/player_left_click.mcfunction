...
scoreboard players operation temp glbs_common = @s glbs_last_action_target_order
   ...
   # 根据玩家点击的控件执行相应功能（目前用序号写死, 可考虑获取看向的控件 id, 便于维护和修改 ui 界面的排序）
      ...
      # 10 >> 添加回收的物品
      execute if score temp glbs_common matches 10 run return run function global_shop:logic/menu/handlers/editmode_menu_handler/handle/player_left_click/add_recycle_item