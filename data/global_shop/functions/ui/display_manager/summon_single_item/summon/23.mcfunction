...
# 写入数据
data modify entity @s item merge from storage global_shop:common g_itemsToDisplay[23]

# 编号
scoreboard players set @s glbs_order 23

# 骑 Menu
ride @s mount @e[type=minecraft:block_display,tag=glbs_to_carry_item_display,distance=..5,limit=1]

# 旋转，让物品显示在玩家前方
scoreboard players set incre glbs_common 36
...
function global_shop:ui/display_manager/summon_single_item/summon/adjust_pos_line_3
