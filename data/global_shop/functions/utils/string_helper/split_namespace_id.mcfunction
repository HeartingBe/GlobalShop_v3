...
# @api
# @参数 `storage global_shop:args splitNamespaceID.namespaceID`
# @返回 `storage global_shop:common return.namespace` 命名空间
# @返回 `storage global_shop:common return.id` ID

data modify storage global_shop:common originalString set from storage global_shop:args splitNamespaceID.namespaceID
...
execute if score temp.success glbs_common matches -1 run data modify storage global_shop:common return.id set from storage global_shop:args splitNamespaceID.namespaceID