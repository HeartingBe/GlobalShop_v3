...
# 类初始化操作，仅调用一次
execute unless score debug.enable glbs_debug_const matches 1 if score g_isInit glbs_common matches 1 as @a if score @s glbs_permission = Permission::ADMIN glbs_common run tellraw @s [" \n\u00a7f\u25a0\u00a77\u25a0\u00a78\u25a0 ","\u00a7dGlobalShop \u00a7a已经初始化了\n     要重新初始化, 请",{"text":"点击此处","color":"yellow","underlined":true,"clickEvent":{"action":"run_command","value":"/function global_shop:settings/re_init"}}]
