...
# 执行到此处说明不是首次安装，而是玩家重复 /reload，
   ...
   # 数据包未运行时，提示管理员开启
   execute unless score g_enable glbs_common matches 1 as @a if score @s glbs_permission = Permission::ADMIN glbs_common run tellraw @s [{"text":"全球商店目前处于停止运行的状态, 可能的原因:","color":"red"},"\n",{"text":"1. 没有完整执行一轮的所有命令(数据可能已损坏)","color":"red"},"\n",{"text":"2. 重装了不同版本的全球商店, 但又换回了正确的版本","color":"red"},"\n",{"text":"3. 手动关闭","color":"red"},"\n",{"text":"4. 新安装, 还未启动过","color":"red"},"\n",{"text":"请确保排查以上问题","color":"red"},{"text":" -> ","color":"red"},{"text":"点击这里启动全球商店","color":"yellow","clickEvent":{"action":"run_command","value":"/function global_shop:settings/try_boot"}}]